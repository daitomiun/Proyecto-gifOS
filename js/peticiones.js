...
async function callApi() {
  ...
  //   var apiKey = "j2lEFfMiqpn38YVWrgdRb3YmJjuJUPnt";

  const elFetch = await fetch(
    `https://api.giphy.com/v1/gifs/search/tags?q=${uInput}&api_key=${apiKey}&limit=1`
  );
...
function deleteGifs() {
  ...
  //   document.getElementById("show-requested").removeChild("written-recomend");
  //   document.getElementById("written-recomend").removeChild("written-recomend");
  //let quitAllMore = document.getElementById("more-button");
  //   requested.removeChild(quitAllWrittenText);
  //   recomend.removeChild(recomend);
  requested.parentNode.removeChild(quitAllWrittenText);
  recomend.parentNode.removeChild(recomend);

  //   requested.removeChild("reflected-input");
  //   recomend.removeChild("written-recomend");

  //   if (requested.parentNode) {
  //     requested.parentNode.removeChild(quitAllWrittenText);
  //   }
  // if (recomend.parentNode) {
  //   recomend.parentNode.removeChild(recomend);
  // }

  //   while (requested.firstChild) {
  //     requested.removeChild(requested.firstChild);
  //   }
...
async function gifCall() {
  ...
  // let maxLimit = 48
  const fetchGif = await fetch(
    `https://api.giphy.com/v1/gifs/search?q=${uInput}&api_key=${apiKey}&limit=${limit}`
  );
...
async function recomendations() {
  ...
  if (uInput == null || uInput.length == 0) {
    ...
    const fetchSugest = await fetch(
      `https://api.giphy.com/v1/gifs/search/tags?q=${uInput}?&api_key=${apiKey}`
    );
...
!(async function trending() {
  ...
  const apiTrendfetch = await fetch(
